#!/usr/bin/env python3
"""
Test that the 2 new model graphs (liquid, mixtral) compile to valid,
runnable Python code via the Rapid Neural Designer's handlebars-style
template compiler.

Reuses the same compiler logic as test_new_models_compile.py.
"""
...
MODEL_GRAPHS = {
    "liquid":  os.path.join(SCRIPT_DIR, "models", "liquid.json"),
    "mixtral": os.path.join(SCRIPT_DIR, "models", "mixtral.json"),
}
...
COMPONENT_PATHS = {
    ...
    "molecular/liquid_cell":            os.path.join(SCRIPT_DIR, "components", "liquid_cell.json"),
    "molecular/moe_transformer_block":  os.path.join(SCRIPT_DIR, "components", "moe_transformer_block.json"),
...
# ---------------------------------------------------------------
# Compiler (identical logic to test_new_models_compile.py)
# ---------------------------------------------------------------
...
def main():
    ...
    print(f"{'=' * 60}")
    if all_pass:
        print("  All 2 models compiled and executed successfully!")
    else:
